...
export default function FindingsExceptions() {
  ...
  return (
    <Box>
        ...
        <TableContainer>
          <Table size="small">
            ...
            <TableBody>
              {findings.length > 0 ? (
                findings.map((item) => (
                  <TableRow key={item.definition.id} hover>
                    <TableCell sx={{ py: 2 }}>
                ...
                <TableRow>
                  <TableCell colSpan={7} sx={{ py: 6, textAlign: 'center' }}>
                    <Typography color="text.secondary" variant="body2" fontWeight={500}>
                      No active findings or compliance gaps recorded.
                    </Typography>
                  </TableCell>
                </TableRow>
              )}
      ...
      {/* Edit Findings Details Dialog */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ fontWeight: 800, pb: 1 }}>
          Findings & Exceptions Details: {selectedControlDef?.id}
        </DialogTitle>
          ...
          <Stack spacing={3.5}>
            ...
            <Box sx={{ p: 2, bgcolor: '#f8fafc', borderRadius: 2.5, border: '1px solid #e2e8f0' }}>
              ...
              <Typography variant="body2" sx={{ color: '#475569', fontSize: '0.85rem', whiteSpace: 'pre-wrap' }}>
                {selectedControlDef ? (wbControls.find((c: any) => c.id === selectedControlDef.id)?.response || 'No response entered in Controls tab.') : ''}
              </Typography>
            </Box>
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button variant="contained" onClick={handleSave} color="primary" sx={{ px: 3 }}>
